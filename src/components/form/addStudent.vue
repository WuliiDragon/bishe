      ...
      <div style="margin-top: 20px;">
        <Form ref="formValidate" :model="formValidate" :rules="ruleValidate" :label-width="80">
          ...
          <FormItem label="学生学号" prop="id">
            <Input v-model="formValidate.id" :disabled="mode != 0" placeholder="学号"></Input>
          </FormItem>
          ...
          <FormItem label="成绩">
            <div v-if="!addGrade">
              <Tag v-for="(item, index) in formValidate.tagList" :key="item" @on-close="delTag(index)" :name="item"
                   closable :color="color[index % 4]">{{item}}
              </Tag>
          ...
          <FormItem>
            ...
            <Button v-if="mode === 0" type="ghost" @click="handleReset('formValidate')" style="margin-left: 8px">重置
            </Button>
          </FormItem>
        </Form>
      </div>
